...
//        for (ZMExpert* exp in SESSION.expertList) {
//            if ([exp.easeId isEqualToString:chatter]) {
//                _expert = exp;
//            }
//        }
...
- (void)viewDidLoad
{
    ...
    if (self.gupiao) {
        ...
        cmdChat.cmd = @"stock";
        cmdChat.params = [NSArray arrayWithObject:[NSDictionary dictionaryWithObject:self.gupiao forKey:@"code"]];
        EMCommandMessageBody * body = [[EMCommandMessageBody alloc] initWithChatObject:cmdChat];
        //            body.action = @"";
        NSLog(@"body.params%@",body.params);
        EMMessage* message = [[EMMessage alloc] initWithReceiver:_chatter bodies:@[body]];
        //        message.messageType = eMessageTypeChat; // 设置为单聊消息
        message.ext = [NSDictionary dictionaryWithObject:self.gupiao forKey:@"code"];
...
- (UITableView *)tableView
{
    if (_tableView == nil) {
        ...
        _tableView.backgroundColor = COLOR_WITH_HEX(0xeeeeee);
...
//            if ([model.message.from isEqualToString:_chatter]) {
//                model.nickName = _expert.show_name;
//                model.headImageURL = [NSURL URLWithString: _expert.avatar];
//            }else{
//                model.nickName = SESSION.loginUser.show_name;
//                model.headImageURL = [NSURL URLWithString:SESSION.loginUser.avatar];
//            }
...
- (NSDictionary*)setExt{
    ...
    [ext setValue:SESSION.loginUser.show_name forKey:@"nick"];
    [ext setValue:SESSION.loginUser.avatar forKey:@"avatar"];
    return ext;
}
...
-(void)sendAudioMessage:(EMChatVoice *)voice
{
    EMMessage *tempMessage = [ChatSendHelper sendVoice:voice
                                            toUsername:_conversation.chatter
                                           isChatGroup:_isChatGroup
                                     requireEncryption:NO ext:[self setExt]];
    [self addMessage:tempMessage];
}
